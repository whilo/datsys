...
(defn selected-columns
  [conn]
  (posh/pull conn '[:table.view.column/attributes] column-selector))
...
(defn type-attribute-tree-reaction
  "Returns a posh reaction of the recursive pull of types defined by type-tree-pull-pattern"
  [conn type-id]
  (posh/pull conn type-tree-pull-pattern type-id))

(defn selected-column-paths
  ([conn base-type-id]
   (selected-column-paths conn base-type-id []))
  ([conn base-type-id base-path]
   (reaction
     (let [type-entity @(type-attribute-tree-reaction conn base-type-id)
...
(defn r-unfolded-type?
  [conn type-id]
  (reaction (unfolded-type? @(posh/pull conn '[:table.view.column.selector/_unfolded-types] type-id))))
...
(defn type-folder
  ...
  (let [type-id (:db/id type-entity)
        unfolded? (r-unfolded-type? conn type-id)]
    (fn [_ type-entity]
      [re-com/h-box
       :gap "3px"
...
(defn attribute-column-selector-row
  ...
  ([conn attr-entity seen]
   (let [checked? (posh/pull conn '[:table.view.column/_attributes] (:db/id attr-entity))]
     [re-com/v-box
                 ...
                 (when @checked?
                   ...
                   [re-com/v-box
                    :children (for [ref-type-id (map :db/id (:attribute.ref/types attr-entity))]
                                ...
                                ^{:key ref-type-id}
                                [attribute-column-selector-rows conn ref-type-id seen])])]])))
...
(defn attribute-column-selector-rows
  ...
  ([conn type-id]
   (attribute-column-selector-rows conn type-id #{}))
  ([conn type-id seen]
   (let [type-entity (type-attribute-tree-reaction conn type-id)
         unfolded? (r-unfolded-type? conn type-id)]
     (fn [_ type-id]
       (let [type-eid (:db/id @type-entity)]
         (if-not (seen type-eid)
           [re-com/v-box
            ...
            :children [[type-folder conn @type-entity]
                       (when (not @unfolded?)
                         [re-com/v-box
                          ...
                          :children [[re-com/v-box
                                      :children (for [subtype-id (map :db/id (:e.type/_isa @type-entity))]
                                                  ^{:key subtype-id}
                                                  [attribute-column-selector-rows conn subtype-id (conj seen type-eid)])]
                                     ...
                                     [re-com/v-box
                                      :children (for [attr-entity (:e.type/attributes @type-entity)]
                                                  ...
                                                  [attribute-column-selector-row conn attr-entity (conj seen type-eid)])]]])]]
       ...
       :children [[re-com/h-box
                  ...
                  (when-not @collapse?
                    [re-com/border
                     ...
                     :child [attribute-column-selector-rows conn (:db/id @(posh/pull conn '[:db/id] base-type))]])]])))
...
;(defn evaluate-query
  ;[conn query-context eids]
  ;(posh/q conn (:query query-context) eids))
         ...
         :children [[re-com/title :level :level2 :label "Table view"]
                    [re-com/h-box
                     ...
                     :children [[re-com/md-icon-button
                                ...
                                [attribute-column-selector conn base-type]]]
                    [:table
                     [:tbody
